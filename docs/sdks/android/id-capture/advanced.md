...
By default, when [IdDocumentType.DL_VIZ] or [IdDocumentType.ID_CARD_VIZ] are selected, *Id Capture* scans only the front side of documents. You may be interested in extracting combined information from both the front and back side.
...
Start by scanning the front side of a document. After you receive the result in [IdCaptureListener], inspect [VizResult.isBackSideCaptureSupported].
...
* [AAMVAVizBarcodeComparisonVerifier]: Validates the authenticity of the document by comparing the data from the VIZ and the barcode on the back.
